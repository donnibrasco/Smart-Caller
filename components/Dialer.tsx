...
import { formatPhoneNumber, formatPhoneForDisplay } from '../utils/phoneUtils';
...
export const Dialer: React.FC<DialerProps> = ({ 
  ...
  const handleManualCall = () => {
    ...
    const formattedPhone = formatPhoneNumber(manualPhoneNumber.trim());
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-3 border-b border-slate-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-bold text-slate-800 flex items-center">
            <Phone size={14} className="mr-1.5 text-blue-600" />
            Quick Dial
          </h3>
          <span className="text-xs text-slate-500">
            {twilioNumbers.find(n => n.id === selectedNumberId)?.label || 'Select number'}
          </span>
        </div>
        <div className="grid grid-cols-12 gap-2">
          <input
            ...
            className="col-span-3 px-2 py-1.5 bg-white border border-slate-300 rounded text-xs focus:ring-2 focus:ring-blue-500 outline-none"
          />
          <input
            type="tel"
            ...
            className="col-span-6 px-2 py-1.5 bg-white border border-slate-300 rounded text-xs focus:ring-2 focus:ring-blue-500 outline-none font-mono"
          />
          <button
            type="button"
            onClick={handleManualCall}
            disabled={!manualPhoneNumber.trim() || twilioNumbers.length === 0}
            className="col-span-3 flex items-center justify-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white text-xs font-bold rounded shadow-sm transition-colors"
          >
            <Phone size={14} />
            <span>Call</span>
          </button>
      ...
      {/* Toolbar - Compact */}
      <div className="p-2 border-b border-slate-200 flex items-center justify-between gap-2 bg-slate-50">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {selectedIds.size > 0 && (
            <button 
              ...
              className="flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 hover:bg-red-200 text-xs font-medium rounded transition-colors"
            >
              <Trash2 size={12} />
          ...
          <div className="relative">
            <div className="flex items-center gap-1.5 bg-white border border-slate-300 rounded px-2 py-1 hover:border-blue-400 transition-colors">
              <Phone size={12} className="text-slate-500" />
              <select 
                value={selectedNumberId} 
                onChange={(e) => onSelectNumber(e.target.value)}
                className="appearance-none bg-transparent text-xs text-slate-700 font-medium outline-none cursor-pointer pr-4 min-w-[100px]"
              >
              ...
              <ChevronDown size={10} className="absolute right-2 text-slate-400 pointer-events-none" />
            </div>
          </div>
          ...
          <button 
            ...
            className="flex items-center gap-1 px-2 py-1 bg-white border border-purple-200 text-purple-700 hover:bg-purple-50 text-xs font-medium rounded transition-colors"
            title="AI Sort"
          >
            <Sparkles size={12} />
          ...
          <button 
            ...
            className={`flex items-center gap-1 px-2 py-1 border text-xs font-medium rounded transition-colors ${
              isParallelMode 
          ...
          >
            <Layers size={12} />
            <span className="hidden md:inline">{isParallelMode ? '3x' : '1x'}</span>
          </button>
          ...
          <button 
            ...
            className="flex items-center gap-1 px-2 py-1 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 text-xs font-medium rounded transition-colors"
          >
            <Upload size={12} />
          ...
          <button 
            ...
            className={`flex items-center gap-1 px-3 py-1 text-white text-xs font-bold rounded shadow-sm transition-transform active:scale-95 ${
              isParallelMode 
      ...
      {/* Header Row - Compact */}
      <div className="grid grid-cols-12 gap-2 px-3 py-2 bg-slate-100 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider items-center">
      ...
      <div className="flex-1 overflow-y-auto bg-white">
        {filteredLeads.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-slate-400">
            ...
            <h3 className="text-sm font-medium text-slate-600">No leads</h3>
            <p className="text-xs text-slate-400 mb-3">Import CSV</p>
            <button 
              type="button"
              onClick={() => fileInputRef.current?.click()}
        ...
        filteredLeads.map((lead) => (
          <div 
            key={lead.id} 
            className={`grid grid-cols-12 gap-2 px-3 py-2 border-b border-slate-100 transition-colors items-center group text-xs ${
          ...
          >
            ...
            <div className="col-span-3 text-slate-600 font-mono truncate">
              {lead.phone}
            </div>
            ...
            <div className="col-span-1 flex justify-end gap-1">
              <button 
                ...
                className="flex items-center justify-center w-6 h-6 bg-white border border-slate-200 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded shadow-sm transition-colors"
                title="Delete"
              >
                <Trash2 size={12} />
              </button>
              <button 
                type="button"
                onClick={() => onCall(lead)}
                className="flex items-center justify-center w-6 h-6 bg-slate-800 hover:bg-slate-900 text-white rounded shadow-sm transition-colors"
                title="Call"
              >
                <Phone size={12} />
              </button>
      ...
      <div className="p-2 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex justify-between items-center">
        ...
        <span>{isParallelMode ? '3x Mode' : '1x Mode'}</span>
      </div>
    </div>
  );
};
